...
Pure-egui, host-free, testable (mirrors `editor_egui`). Own `MonoUiState` mirror + `apply(EngineEvent)` (drum `UiState` is track-shaped, not reusable — PR #20 FeelBar + PR #21 TrackManagementBar made it more drum-specific). `CommandSink` trait re-cloned from `editor_egui/src/lib.rs:14`.
