...
# Modern Xcode rejects two separate simulator slices (arm64-sim + x86_64-sim) as
# "equivalent library definitions". Merge them into one universal simulator
# archive with lipo, then package device + universal-sim — the standard
# Rust-on-iOS xcframework pattern.
echo "[build] merging simulator slices (lipo) ..."
