...
fn commands_emit_events() {
    ...
    // SetSyncSource
    eng.apply_command(Command::SetSyncSource { source: SyncSource::Link });
    let ev = eng.hot_events.dequeue().expect("SetSyncSource should emit an event");
    let ev: EngineEvent = postcard::from_bytes(&ev.bytes[..ev.len as usize]).unwrap();
    assert_eq!(ev, EngineEvent::SyncSourceChanged { source: SyncSource::Link });
}
